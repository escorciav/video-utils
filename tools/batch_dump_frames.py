...
def dump_wrapper(filename, output_folder, baseformat, extra_filters,
    ...
    return dump_frames(filename, output_dir, baseformat, extra_filters)
...
def input_parser():
    ...
    p.add_argument('-bf', '--baseformat', default='%06d.png',
                   help='Format used for naming frames e.g. %%06d.jpg')
    p.add_argument('-ef', '--extra-filters', default='',
...
def main(input_file, output_folder, baseformat, extra_filters, n_jobs,
         fullpath_flag, video_path):
    df = pd.read_csv(input_file, sep='\t', header=None)
    Parallel(n_jobs=n_jobs)(
        delayed(dump_wrapper)(i, output_folder, baseformat, extra_filters,
                              fullpath_flag, video_path)
        for i in df.loc[:, 0])
    return None
